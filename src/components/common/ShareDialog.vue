...
const emit = defineEmits<{
  ...
  share: [settings: { accessMode: ShareAccessMode }]
}>()
...
watch(() => props.open, (isOpen) => {
  if (isOpen) {
    const settings = shareStore.getShareSettings(props.itemId)
...
watch(accessMode, (newMode) => {
  shareStore.updateWorkflowAccessMode(props.itemId, newMode)
})
...
function generateOrCopyLink() {
  if (!shareLink.value) {
    shareStore.createShareLink(props.itemId, accessMode.value)
  }
  copyLink()
}
