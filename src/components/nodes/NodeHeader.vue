...
<template>
  <div
    ...
    style="background-color: var(--node-header-bg)"
    @dblclick="handleDoubleClick"
  >
    <div class="flex items-center gap-1.5 min-w-0">
      <button
        class="flex h-4 w-4 shrink-0 items-center justify-center text-muted-foreground transition-colors hover:text-foreground"
      ...
      <div class="flex min-w-0 flex-1 items-center">
        <input
          ...
          v-model="editValue"
          type="text"
          class="w-full min-w-0 truncate bg-transparent text-xs font-medium text-foreground outline-none ring-1 ring-blue-500 rounded px-1"
          autofocus
      ...
      <div v-if="badges?.length || statusBadge || pinned" class="flex shrink-0 items-center gap-1">
        ...
        <Icon v-if="pinned" name="thumbtack" size="xs" class="text-muted-foreground" />
      </div>
    </div>
  </div>
</template>
